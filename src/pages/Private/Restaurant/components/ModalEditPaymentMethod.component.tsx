...
export const ModalEditPaymentMethod = NiceModal.create<Props>(
  ({ paymentMethod }) => {
    ...
    } = useForm<UpdatePaymentMethodDto>({
      values: {
        name: paymentMethod.name,
        type: paymentMethod.type,
        commissionPercentage: paymentMethod.commissionPercentage ?? undefined,
        defaultDestinationAccountId:
          paymentMethod.defaultDestinationAccount?.id,
        allowedDestinationAccountIds:
          paymentMethod.allowedDestinationAccounts?.map((a: Account) => a.id) ??
          []
      }
    });
    ...
    // Merge prop accounts (immediately available) with query accounts (full data).
    // Query data wins on dedup so `type` field is always present.
    const accountPool: Account[] = [
      ...accounts,
      ...(paymentMethod.allowedDestinationAccounts ?? []),
      ...(paymentMethod.defaultDestinationAccount
        ? [paymentMethod.defaultDestinationAccount]
        : [])
    ].filter((a, idx, arr) => arr.findIndex((b) => b.id === a.id) === idx);

    // Options filtered by payment type — for new selections
    const accountTypeFilter =
      paymentType === PaymentMethodCategory.CASH
        ? AccountType.CASH
        : AccountType.BANK;
    const filteredAccounts = accountPool.filter(
      (a) => a.type === accountTypeFilter
    );

    // Currently selected allowed accounts (for the default picker section)
    const selectedAccounts = accountPool.filter((a) =>
      allowedIds.includes(a.id)
    );

    // Reset account selections only when the user actively changes the type,
    // not on the initial sync from `values`.
    const prevTypeRef = useRef<PaymentMethodCategory | undefined>(
      paymentMethod.type
    );
    useEffect(() => {
      if (!paymentType || prevTypeRef.current === paymentType) return;
    ...
    const onSubmit = (data: UpdatePaymentMethodDto) => {
      mutateAsync({ id: paymentMethod.id, dto: data }).then(() => modal.hide());
    };

    return (
      <Dialog {...muiDialogV5(modal)} fullWidth maxWidth='sm'>
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle variant='h5'>Editar método de pago</DialogTitle>
            ...
            <Controller
              ...
              render={({ field }) => (
                <FormControl fullWidth sx={{ mt: 1 }}>
                  <InputLabel id='pm-edit-type-label'>Tipo</InputLabel>
                  <Select
                    labelId='pm-edit-type-label'
                    label='Tipo'
                    {...field}
                    error={!!errors.type}
                  >
